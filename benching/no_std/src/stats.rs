...
fn local_sort(v: &mut [f64]) {
    v.sort_unstable_by(|x: &f64, y: &f64| x.total_cmp(y));
}
...
impl Summary {
    ...
    pub fn new<const N: usize>(samples: &[f64; N]) -> Summary {
        Summary {
            sum: samples.sum(),
            min: samples.min(),
            max: samples.max(),
...
impl<const N: usize> Stats for [f64; N] {
    // FIXME #11059 handle NaN, inf and overflow
    fn sum(&self) -> f64 {
        let mut partials = heapless::Vec::<f64, N>::new();
        ...
        for &x in self {
            ...
            if j >= partials.len() {
                ...
                partials[j] = x;
                partials.extend_one((j + 1) as f64);
            }
        }
        let zero: f64 = 0.0;
    ...
    fn min(&self) -> f64 {
        ...
        self.iter().fold(self[0] as f64, |p, q| p.min(*q as f64))
    }

    fn max(&self) -> f64 {
        assert!(!self.is_empty());
        self.iter().fold(self[0] as f64, |p, q| p.max(*q as f64))
    }
    ...
    fn percentile(&self, pct: f64) -> f64 {
        let mut tmp = heapless::Vec::<f64, N>::from_slice(self).unwrap();
        local_sort(&mut tmp);
